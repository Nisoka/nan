...
void QCPAxis::mZoomOutRectRange(double lower,double upper)
{
    QCPRange oldRange = mRange;
...
void QCustomPlot::mousePressEvent(QMouseEvent *event)
{
  ...
  //qDebug()<<"77777777777777777777777777777777777777777777777";
...
void QCustomPlot::mouseReleaseEvent(QMouseEvent *event)
{
 ...
 // qDebug()<<"88888888888888888888888888888888888888888888";
...
void QCPAxisRect::ZoomInOut(int flag,QPoint zoomPosition)
{
    if (mParentPlot->interactions().testFlag(QCP::iRangeZoom))
    {
        if (mRangeZoom != 0)
        {
            ...
            double factor;
            double wheelSteps = 1.0; // a single step delta is +/-120 usually
            ...
            // printf("mDragStart.x()=%d mDragStart.y()=%d \n ",mDragStart.x(),mDragStart.y());
            // qDebug()<<"mDragStart.x()="<<mDragStart.x()  << "mDragStart.y()="<<mDragStart.y() ;
            if (mRangeZoom.testFlag(Qt::Horizontal))
            {
                factor = qPow(mRangeZoomFactorHorz, wheelSteps);
                if (mRangeZoomHorzAxis.data())
                    mRangeZoomHorzAxis.data()->scaleRange(factor, mRangeZoomHorzAxis.data()->pixelToCoord(zoomPosition.x()));
            }
            if (mRangeZoom.testFlag(Qt::Vertical))
            {
                factor = qPow(mRangeZoomFactorVert, wheelSteps);
                if (mRangeZoomVertAxis.data())
                {
                    //printf("factor = %f, center = %f\n", factor, mRangeZoomVertAxis.data()->pixelToCoord(zoomPosition.y()));
                    mRangeZoomVertAxis.data()->scaleRange(factor, mRangeZoomVertAxis.data()->pixelToCoord(zoomPosition.y()));
                }
            }
            mParentPlot->replot();
        }
...
void QCPAxisRect::ZoomInRect(double startKey,double startValue,double endKey,double endValue)
{
    double temp;
    if(endKey < startKey)
    {
    ...
    //printf("startKey=%f endKey=%f\n",startKey,endKey);
    if (mRangeZoomHorzAxis.data())
        mRangeZoomHorzAxis.data()->mZoomOutRectRange(startKey, endKey);

   // printf("startValue=%f endValue=%f\n",startValue,endValue);
    if (mRangeZoomVertAxis.data())
        mRangeZoomVertAxis.data()->mZoomOutRectRange(startValue,endValue);
...
void QCPGraph::rescaleValueAxis(bool onlyEnlarge, bool includeErrorBars) const
{
 ...
 void QCPGraph::mydraw(QCPPainter *painter)
 {
//        painter->drawLine();
     painter->setBrush(QBrush(QColor(178, 206, 240, 120),Qt::SolidPattern)); //设置画刷形式

     painter->setPen(QColor(178, 206, 240, 120));

     QRect rubberBandRect(30,30,30,30);

     painter->drawRect(rubberBandRect);
 }
 QCPRange QCPGraph::mgetKeyRange(void)
 {
     //qDebug()<< "mmmmmmmmmmmmmm maxRange="<<getKeyRange(false,QCPAbstractPlottable::sdBoth,false).maxRange();
     QCPRange range;
...
void QCPGraph::getLinePlotData(QVector<QPointF> *linePixelData, QVector<QCPData> *scatterData) const
{
  ...
  //{
      //qDebug()<<"key"<< lineData.at(i).key <<"value"<<lineData.at(i).value;
  //}
...
void QCPGraph::getVisibleDataRange(double &keylower, double &keyupper,double &valuelower, double &valueupper)
{
  if (!mKeyAxis) { qDebug() << Q_FUNC_INFO << "invalid key axis"; return; }
  if (mData->isEmpty())
  {
  ...
  keylower=mKeyAxis.data()->range().lower;
  keyupper=mKeyAxis.data()->range().upper;

  valuelower=mValueAxis.data()->range().lower;
  valueupper=mValueAxis.data()->range().upper;
  ...
  switch (mStyle)
  {
    ...
    {        
     // if (center.y() > clip.top() && center.y() < clip.bottom())
      ...
      if (center.x() > clip.left() && center.x() < clip.right())
      {
          //QFont temp_bak=painter->font();

          QFont temp_font("WenQuanYi Zen Hei");
          temp_font.setPixelSize(12);
          temp_font.setStyleStrategy(QFont::PreferQuality);
          temp_font.setBold(true);
          painter->setFont(temp_font);
         // QPixmap pixR(14,18);
         // pixR.load(tsCrosshairBlackgroudDir);
         // painter->drawPixmap(center.x(),clip.top(),pixR.width(),pixR.height(),pixR);
          painter->drawText(center.x()+3,clip.top()+ 10+tsCrosshairTextYoffset,tsCrosshairText);
    ...
    case tsEventMark:
    {
       ...
       {
           QPixmap pixR(20,32);
           pixR.load(tsEventMarkPNGDir);
           painter->setRenderHint(QPainter::SmoothPixmapTransform,true);
           painter->drawPixmap(center.x()-pixR.width()/2-1,center.y(),pixR.width(),pixR.height(),pixR);

           QFont temp_font("WenQuanYi Zen Hei");
           temp_font.setPixelSize(12);
           temp_font.setStyleStrategy(QFont::PreferQuality);
           painter->setFont(temp_font);
          // painter->drawText(center.x()-4,center.y()+27,tsEventNo);
           //居中显示事件号
           QRectF textRect(center.x()-pixR.width()/2-1,center.y()+pixR.height()/2-2,pixR.width(),pixR.height()/2);
           painter->drawText(textRect,Qt::AlignCenter,tsEventNo);

       }
       break;
    }
  case tsSectionEventMark:
  {
      ...
      if (center.x() > clip.left() && center.x() < clip.right())
      {
          ...
          pixR.load(tsEventMarkPNGDir);
          painter->setRenderHint(QPainter::SmoothPixmapTransform,true);
          painter->drawPixmap(center.x(),center.y()-pixR.height()/2,pixR.width(),pixR.height(),pixR);
          //painter->drawText(center.x()-4,center.y()+ 27,tsEventNo);
      }
      break;
  }
